...
function registerGeneratorCommand(context: vscode.ExtensionContext): void {
    ...
    context.subscriptions.push(vscode.commands.registerCommand("tolkien.generate",callback));
...
function startLanguageClient(context: vscode.ExtensionContext): LanguageClient {
    ...
    const fileSystemWatcher = vscode.workspace.createFileSystemWatcher('**/*.tolkien');
    context.subscriptions.push(fileSystemWatcher);

    // Options to control the language client
    const clientOptions: LanguageClientOptions = {
        documentSelector: [{ scheme: 'file', language: 'tolkien' }],
        synchronize: {
